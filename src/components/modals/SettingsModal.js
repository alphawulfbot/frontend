...
const SettingsModal = ({ onClose }) => {
    ...
    const [soundEffects, setSoundEffects] = (0, react_1.useState)(true);
    return (<framer_motion_1.AnimatePresence>
      <framer_motion_1.motion.div className="fixed inset-0 bg-black bg-opacity-80 z-50 flex items-center justify-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}>
        <framer_motion_1.motion.div className="bg-[#1f1f1f] border-2 border-[#ffd700] rounded-lg w-[90%] max-w-[400px] p-5" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} onClick={e => e.stopPropagation()}>
