...
const Wallet = () => {
    const { coinBalance } = (0, store_1.useStore)();
    const [selectedAmount, setSelectedAmount] = (0, react_1.useState)(null);
    ...
    return (<div className="py-4">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          {withdrawalOptions.map((option) => (<framer_motion_1.motion.button key={option.amount} className={`card p-4 text-center ${selectedAmount === option.coins ? 'border-2 border-[#ffd700]' : ''}`} onClick={() => setSelectedAmount(option.coins)} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
              <p className="text-lg font-bold">{option.amount} INR</p>
              <p className="text-sm text-gray-400">{option.coins} coins</p>
            </framer_motion_1.motion.button>))}
        </div>

        <framer_motion_1.motion.button className={`withdraw-btn w-full ${!selectedAmount || coinBalance < selectedAmount ? 'disabled' : ''}`} onClick={handleWithdraw} disabled={!selectedAmount || coinBalance < selectedAmount} whileHover={selectedAmount && coinBalance >= selectedAmount ? { scale: 1.02 } : {}} whileTap={selectedAmount && coinBalance >= selectedAmount ? { scale: 0.98 } : {}}>
          {!selectedAmount
            ? 'Select an amount'
